	...
	public class LootGeneratorUniqueObject : LootGeneratorBase
	{
		...
		public class LootGeneratorUniqueObjectCommandHandler : DOL.GS.Commands.AbstractCommandHandler, DOL.GS.Commands.ICommandHandler
		{
			public void OnCommand(GameClient client, string[] args)
			{
				...
				mob.CurrentRegionID = 1; // I think chriswf changed this from 30 to 1...
		...
		public override LootList GenerateLoot(GameNPC mob, GameObject killer)
		{
			...
			if (mob.CurrentZone.IsDivingEnabled && mob.Z < mob.CurrentZone.Waterlevel)
			{
				// no ROG for underwater mobs
				return loot;
			}

			int baseChance = Math.Min(30, Math.Max(3, 100 - (mob.Level * 4)));
			int chance = baseChance + (int)killer.GetConLevel(mob);
			...
			if (killer.Realm != 0 && Util.Chance(chance))
			{
				...
				if (newItem != null)
				{
					newItem.Description = "Unique Object";
		...
		private static eObjectType GenerateObjectType(int realm, int level)
		{
			...
			else if (Util.Chance(15)) type = eGenerateType.Magical;
				...
				if (Util.Chance(item.Level * 8 - 40)) // level 6 - 17 (100%)
				{
					...
					if (Util.Chance(item.Level * 6 - 60)) // level 11 - 27 (100%)
					{
						...
						if (Util.Chance(item.Level * 4 - 80)) // level 21 - 45 (100%)
						{
							...
							if (number == 4 && item.Level > 50) //  chriswf removed: && IsMobInTOA(mob) 
                                number++; // 5
						}
					}
				}
		...
		private static eBonusType GetPropertyType(ItemUnique item, GameNPC mob)
		{
			...
			if ( mob.Level > 50 && Util.Chance(10))
			{
