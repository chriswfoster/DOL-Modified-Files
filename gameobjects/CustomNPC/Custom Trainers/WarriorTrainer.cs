...
namespace DOL.GS
{
    ...
    public class WarriorTrainer : GameNPC
    {
        ...
        public override bool Interact(GamePlayer player)
        {
            ...
            {
                SayTo(player, "Hey there, do you want to learn about the [warrior] class?");
...

        {
            ...
            if (text.ToLower() == "warrior")
            {
                player.Out.SendMessage(String.Format("The Warrior class is a solid, melee dps class. It has decent armor, but no ranged abilities. It can have defensive abilities, but no shields. Expect to utilize 2h weapons most of your career. The Warrior class can swing their weapons and sometimes hit other targets."), eChatType.CT_Say, eChatLoc.CL_PopupWindow);
                player.Out.SendMessage(String.Format("You must be a Norseman or Highlander to be a true Warrior"), eChatType.CT_Say, eChatLoc.CL_PopupWindow);
                player.Out.SendMessage(String.Format("Would you like to [become a warrior] today?"), eChatType.CT_Say, eChatLoc.CL_PopupWindow);

            }
            if (text.ToLower() == "become a warrior" && (player.Race == 3 || player.Race == 5))
                {
                player.Out.SendMessage("Great choice, Warrior! Off you go then!", DOL.GS.PacketHandler.eChatType.CT_Staff, DOL.GS.PacketHandler.eChatLoc.CL_SystemWindow);
                SetClass(player, 22);
            ...
            if (text.ToLower() == "become a warrior" && (player.Race != 3 || player.Race != 5))
            {
                player.Out.SendMessage("Hah, you can't be a Warrior. You must be a Norseman or Highlander!", DOL.GS.PacketHandler.eChatType.CT_Staff, DOL.GS.PacketHandler.eChatLoc.CL_SystemWindow);
            }
