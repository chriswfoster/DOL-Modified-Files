...
namespace DOL.GS
{
	...
	public class SimpleTeleporter : GameTeleporter
	{
		...
		public override bool Interact(GamePlayer player)
		{
            ...
            {
                player.Out.SendMessage(String.Format("Albion Destinations:"), eChatType.CT_Say, eChatLoc.CL_PopupWindow);
            }

            foreach (Teleport destination in m_destinations)
            {
                if (destination.Realm == 1)
                {
